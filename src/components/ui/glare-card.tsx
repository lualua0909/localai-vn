...
export const GlareCard = ({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "grid origin-center overflow-hidden [grid-area:1/1] [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] transition-transform delay-[var(--delay)] duration-[var(--duration)] ease-[var(--easing)] will-change-transform hover:filter-none hover:[--duration:200ms] hover:[--easing:linear] hover:[--opacity:0.6]"
        )}
      >
        <div className="grid w-full [clip-path:inset(0_0_0_0_round_var(--radius))] [grid-area:1/1] mix-blend-soft-light">
        ...
        <div
          className="relative grid h-full w-full opacity-[var(--opacity)] mix-blend-color-dodge transition-opacity [background:var(--pattern),_var(--rainbow),_var(--diagonal),_var(--shade)] [background-blend-mode:hue_hue_hue_overlay] [clip-path:inset(0_0_1px_0_round_var(--radius))] [grid-area:1/1] will-change-background after:grid-area-[inherit] after:bg-[inherit] after:bg-repeat-[inherit] after:bg-attachment-[inherit] after:bg-origin-[inherit] after:bg-clip-[inherit] after:mix-blend-exclusion after:content-[''] after:[background-position:center,_0%_var(--bg-y),_calc(var(--bg-x)*_-1)_calc(var(--bg-y)*_-1),_var(--bg-x)_var(--bg-y)] after:[background-size:var(--foil-size),_200%_400%,_800%,_200%] after:[background-blend-mode:soft-light,_hue,_hard-light]"
